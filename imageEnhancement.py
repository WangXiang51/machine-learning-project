...
def ImageEnhancement(img,name,path):

    #直方图均衡化
    equal = equalHist(img)
    # cv.imshow("img", img)
    # cv.imshow("equal",equal)
    cv.imwrite(path+'/equal/'+str(name)+'.png', equal)
    #拉普拉斯算子滤波
    ...
    cv.imwrite(path + '/laplace/' + str(name) +'.png', laplace)
    equalLaplace = cv.filter2D(equal, -1, kernel_sharpen_1)
    cv.imwrite(path + '/equal+laplace/'+ str(name) + '.png',equalLaplace)
    ...
    cv.imwrite(path + '/equal+laplace+img_median/' + str(name) + '.png', img_median)
...
train_size=25
test_size=5
path='./data/new train set'
for i in range(train_size):
    img = cv.imread(path+'/origin_train_img/'+str(i)+".png", 0)
    ImageEnhancement(img, i, path)
